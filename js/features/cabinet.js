...
export const Cabinet = {
    ...
    replaceCabinetCard(indexToRemove, newCardItem, fromButton) {
        ...
        this.addToDisplayCabinet(newCardItem.card.id, fromButton, true, newCardItem.layers, newCardItem.numbering);
        UIElements.cabinetModal.classList.add('hidden');
    },

    addToDisplayCabinet(cardId, buttonElement, isReplacing = false, forcedLayers = null, forcedNumbering = null) {
        // Get layers from forced, button dataset, or generate new ones
        let cardLayers = forcedLayers;
        if (!cardLayers && buttonElement && buttonElement.dataset.layers) {
            cardLayers = JSON.parse(buttonElement.dataset.layers);
        }
        // If still no layers, generate them using the card ID
        if (!cardLayers) {
            cardLayers = CardVisuals.generateLayerIndices(cardId);
        }
        
        // Get numbering from forced, button dataset, or generate if applicable
        let numbering = forcedNumbering;
        if (!numbering && buttonElement && buttonElement.dataset.numbering) {
            numbering = JSON.parse(buttonElement.dataset.numbering);
        }
        // If still no numbering and card is eligible, generate it
        const cardDetails = GameState.getCardDetails(cardId);
        if (!numbering && ['numbered_legend', 'numbered_rookie_auto', 'autographed_jersey'].includes(cardId)) {
            numbering = CardVisuals.generateCardNumbering(cardDetails);
        }
        
        let currentMarketPrice = GameState.market[GameState.current.currentLocationId]?.[cardId]?.price || cardDetails.basePrice;
